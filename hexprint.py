...
@click.option(
    ...
    "--nbytes",
    default=0,
    help="# bytes to display. " + "default = 0 (entire file)",
    metavar="<int>",
)
@click.option(
    "-o",
...
def hexdump(*, filename=None, offset=0, totbytes=0):
    ...
    try:
        ...
        click.echo(
            click.style(
                "Can not open file: " + click.format_filename(filename), fg="red"
            )
        )
        return 0
    ...
    if true_offset % 16 != 0:
        ...
        click.echo(
            click.style(format("%0.8X" % row_offset) + "  ", fg="cyan"), nl=False
        )
        row_string = ""
...
class TempTestFile:
    ...
    def __repr__(self):
        return "<" + (
            self.__class__.__name__ + " object, filename = " + self.filename + ">"
        )
...
def test_cli_values():
    ...
    testcases = [
        ...
        [
            "00000000                 69 73 20-61 20 74 65 73 74 2E          is a test.",
            "-o5",
        ],
        [
            "00000000        69 73 20 69 73                               is is",
            "-o2",
            "-n5",
        ],
        [
            "00000000                    73 20-61 20 74                       s a t",
            "-o-9",
            "-n5",
        ],
    ]
    ...
    with TempTestFile("This is a test.") as tempfilename:
        ...
        for testcase in testcases:
            args = (
                [tempfilename, *testcase[1:]] if len(testcase) > 1 else [tempfilename]
            )
